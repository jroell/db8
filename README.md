...
- **Panel judging.** A Claude judge and a Codex judge deliberate independently every round, score logic, evidence, rebuttal, and persuasion, track the cruxes of the disagreement, and hand each side a specific focus directive for the next round. The debate ends when both are beyond persuasion or the round cap hits. Split decisions are a signal in themselves.
- **Judge conferences.** When the judges land on opposite leanings they confer: each sees the other's deliberation, names what kind of disagreement it is (facts, weights, or what the motion even requires), proposes a neutral shared standard, and revises only if genuinely moved. Politeness convergence is explicitly forbidden, so a split that survives the conference is a real one, crystallized rather than accidental.
